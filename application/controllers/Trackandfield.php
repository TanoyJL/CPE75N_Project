<?php
	...
	class trackandfield extends CI_Controller {
		...
		$this->load->view("players/trackandfield_form",$data);
		...
		function view(){
			...
			$data['sport'] = 'trackandfield Coach';	
		...
		function view_details($id){
			...
			if($query = $this->trackandfield_model->get_record($id)) 	{
					$data['records'] = $query;
			}
			...
			$data['applicants'] = 'New Applicants'	;
			$this->load->view("templates/header3",$data);
			$this->load->view("players/trackandfield_viewdetails", $data);
			$this->load->view("templates/footer");
		...
		function view_approved(){
				$data = array();
				$approval ='approved';
			if($query = $this->trackandfield_model->get_records($approval)) 	{
					$data['records'] = $query;
			}
	...
	public function delete_row($id_num) {   
      $this->load->model("trackandfield_model");
      $this->trackandfield_model->delete_row($id_num);
