<?php
	...
	class Tabletennis_model extends CI_Model{
		...
		function add_records($data){
			...
			$data = array(
				...
				'preferred_category_singles'=> $this->input->post('preferred_category_singles'),
			...
			$this->db->insert('tabletennis',$data);
		...
		function update_records($data){
			$this->db->where('id', 14);
		 	$this->db->update('tabletennis', $data);
		}

		 function delete_row(){
			$this->db->where('id', $this->url->segment(3));
		 	$this->db->delete('tabletennis');
		}
